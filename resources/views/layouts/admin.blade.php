...
<body class="font-sans antialiased bg-gradient-to-br from-violet-400 to-violet-500 dark:from-violet-900 dark:to-slate-900 min-h-screen text-white">

<div class="flex min-h-screen">

    {{-- Sidebar --}}
    <aside class="w-64 flex-shrink-0 bg-black/30 backdrop-blur-xl border-r border-white/10 flex flex-col">
        {{-- Logo --}}
    ...
    <div class="flex-1 flex flex-col min-w-0">
        {{-- Top bar --}}
        <header class="h-16 bg-black/20 backdrop-blur-md border-b border-white/10 flex items-center px-8 flex-shrink-0">
            <div class="flex-1">
                {{ $header ?? '' }}
            </div>
        </header>
        ...
        {{-- Flash messages --}}
        @if(session('success'))
            <div class="mx-8 mt-4 bg-green-500/20 border border-green-400/40 text-green-200 px-4 py-3 rounded-xl text-sm">
                {{ session('success') }}
            </div>
        @endif
        @if(session('error'))
            <div class="mx-8 mt-4 bg-red-500/20 border border-red-400/40 text-red-200 px-4 py-3 rounded-xl text-sm">
                {{ session('error') }}
            </div>
        @endif
